...
impl GithubReleases {
    fn make_headers(&self, mime: Option<&str>) -> HeaderMap {
...
impl Source for GithubReleases {
    ...
    async fn get(&self, descriptor: &Descriptor) -> Result<Vec<u8>, String> {
        ...
        let url = format!("https://github.com/{}/{}/releases/download/{}/d.bin", self.owner, self.repo, tag);
    ...
    async fn put(&self, descriptor: &Descriptor, data: Vec<u8>) -> Result<(), String> {
        ...
        let response = client
            .post(&url)
            .headers(self.make_headers(Some("application/octet-stream")))
            .body(data)
            .send()
            .await
            .map_err(|e| format!("Error sending request: {}", e))?;
    ...
    async fn create(&self) -> Result<Descriptor, String> {
        ...
        let response = client
            .post(&url)
            .headers(self.make_headers(Some("application/json")))
            .body(json!({
                "tag_name": descriptor,
                "name": descriptor,
                "body": "",
